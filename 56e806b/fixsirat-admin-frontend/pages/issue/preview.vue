<template>
  <div>
    ...
    >
        ...
        <form class="form mb-4" @submit.prevent="formValidateHandler()">
          ...
          <div v-if="isEditissue">
            <label for="issueStatus" class="label">
              <span class="label-text">Issue Status</span>
            </label>
            <select
              id="issueStatus"
          ...
          <div v-if="isEditissue">
            <label
              ...
              class="label"
            >
              <span :class="['label-text','text-gray-700' ]">Issue Description</span>
            </label>
            <textarea
              id="issueDescription"
      ...
      <div v-if="Error" class="absolute top-0 right-0 hideAlert">
...
async function submitIssueToServer (): Promise<void> {
  ...
  const body: issueRequestBody = {
    userName: temporaryIssue.value.userName,
...
const submitBtnClasses = computed(() => {
  const submitBtnClasses = ['btn btn-block mt-8', 'btn-warning'];
  loading.value && submitBtnClasses.push('loading');
  Error.value && submitBtnClasses.push('btn-error');
