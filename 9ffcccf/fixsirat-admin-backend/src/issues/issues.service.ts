...
export class IssuesService {
  ...
  getSortingCriteria(sort: string) {
    const sortingCriterias = {
      severity: { severity: SORT_ASC },
      dateTaken: { dateTaken: SORT_DESC },
      distance: { 'dist.calculated': SORT_ASC },
    };
  ...
  async findAllByReporter(reporter: string): Promise<Issue[]> {
    let query = this.issueModel.aggregate();
